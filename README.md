...
- All your project code into a single main package (`meetnote/`)
...
```bash
<root directory>
├── meetnote/             # main package (should be named after your project)
...
1. Use this template to create a new GitHub repository, say `meetnote`
...
2. Clone the `meetnote` repository

3. Open a shell into your local `meetnote` directory and run
    ```bash
    ./rename-template.sh meetnote
    ```
    
    This will coherently rename the template's project name with the one chosen by you (i.e. `meetnote`, in this example)
...
13. Put your main (resp. test) code in `meetnote/` (resp. `test/`)
...
This will execute the `__main__.py` file in the `meetnote` package:
```bash
poetry run python -m meetnote
```
